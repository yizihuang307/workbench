...
export default function RecordsView({ store, setStore, storageError, onNotice }: Props) {
  ...
  const visibleSaveState = storageError ? "error" : saveState;
  ...
  return <div className="records-page">
    ...
    {deleteTarget && <Dialog title="删除记录" onClose={() => setDeleteTarget(null)}><div className="confirm-delete"><p>确定删除这条记录吗？删除后仍可在 5 秒内撤销。</p><footer><button onClick={() => setDeleteTarget(null)}>取消</button><button className="danger" onClick={() => { const item = deleteTarget; setDeleteTarget(null); deleteRecord(item); }}>确认删除</button></footer></div></Dialog>}
