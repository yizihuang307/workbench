...
export default function InformationView({ store, setStore, storageError, onNotice }: Props) {
  ...
  useEffect(() => {
    function key(event: KeyboardEvent) { if (event.key === "Escape") { if (editingId) closeEditor(); else { setManageOpen(false); setNewSystem(false); } } }
  ...
  function createResource(sectionId: string) {
    ...
    const now = Date.now(), item: ResourceItem = { id: infoId(), sectionId, title: "未命名资料", blocks: [{ id: infoId(), type: "text", text: "" }], pinned: false, createdAt: now, updatedAt: now };
  ...
  return <div className="info-page">
    ...
    {systemSection && <section className="system-section" id={`info-${systemSection.id}`}>
      <header><div><h2>{systemSection.name}</h2><span>共 {store.systems.length} 项</span></div><button onClick={() => setNewSystem(true)} aria-label="新增常用系统">＋</button></header>
      <div className="system-grid">{store.systems.sort((a,b) => a.order-b.order).map((item) => {
        ...
        return <article className="system-row" key={item.id}><button className="system-open" onClick={() => window.open(target.url, "_blank", "noopener,noreferrer")} title={item.name}><b>{item.icon}</b><span>{item.name}</span><small>{new URL(target.url).hostname}</small></button><button className="system-remove" aria-label={`删除${item.name}`} onClick={() => { if (window.confirm(`确定删除“${item.name}”吗？`)) setStore((current) => ({ ...current, systems: current.systems.filter((system) => system.id !== item.id) })); }}>···</button></article>;
    ...
    <div className="resource-sections">{resourceSections.map((section) => {
      ...
      return <section className="resource-section" id={`info-${section.id}`} key={section.id}><header><div><h2>{section.name}</h2><span>共 {items.length} 项</span></div><button onClick={() => createResource(section.id)} aria-label={`在${section.name}中新建资料`}>＋</button></header><div className="resource-preview">{items.slice(0,5).map((item) => <ResourceRow key={item.id} item={item} onOpen={() => openEditor(item.id)} onPin={() => setStore((current) => ({ ...current, resources: current.resources.map((resource) => resource.id === item.id ? { ...resource, pinned: !resource.pinned, updatedAt: Date.now() } : resource) }))} />)}{!items.length && <div className="info-empty compact"><span>{query ? "没有匹配结果" : "这里还没有资料"}</span></div>}</div>{items.length > 5 && <button className="view-all" onClick={() => onNotice(`${section.name}还有 ${items.length - 5} 项，可使用搜索快速定位`)}>查看全部 <span>→</span></button>}</section>;
    })}</div>

    {newSystem && <SystemDialog onClose={() => setNewSystem(false)} onSave={(input) => {
      ...
      setStore((current) => ({ ...current, systems: [...current.systems, { id: infoId(), sectionId: systemSection.id, name: meta.name, icon: meta.icon, links: [{ id: linkId, url: meta.url, label: "主页" }], defaultLinkId: linkId, order: current.systems.length, createdAt: now, updatedAt: now }] })); setNewSystem(false); onNotice("常用系统已添加");
    }} />}
...
function ResourceWorkspace({ item, sections, saveState, fileBytes, onClose, onDelete, onUpdate, onNotice }: { item: ResourceItem; sections: InfoSection[]; saveState: string; fileBytes: number; onClose: () => void; onDelete: () => void; onUpdate: (updater: (item: ResourceItem) => ResourceItem) => void; onNotice: (message: string) => void }) {
  ...
  function blocks(next: InfoBlock[]) { onUpdate((current) => ({ ...current, blocks: next, title: current.title === "未命名资料" ? deriveTitle(next) : current.title, updatedAt: Date.now() })); }
...
function SystemDialog({ onSave, onClose }: { onSave: (value:string)=>void; onClose:()=>void }) { const [value,setValue]=useState(""); return <div className="info-dialog-backdrop"><section className="info-dialog small" role="dialog" aria-modal="true" aria-label="新增常用系统"><button className="info-dialog-close" onClick={onClose} aria-label="关闭">×</button><h2>新增常用系统</h2><p>只需粘贴网址，名称和图标会自动生成。</p><input autoFocus value={value} onChange={(event)=>setValue(event.target.value)} onKeyDown={(event)=>event.key==="Enter"&&onSave(value)} placeholder="https://example.com" /><footer><button onClick={onClose}>取消</button><button className="primary" disabled={!safeUrl(value)} onClick={()=>onSave(value)}>添加</button></footer></section></div>; }
